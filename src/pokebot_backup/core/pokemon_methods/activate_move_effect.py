    ...
    match move.name * defender_mgr.can_receive_move_effect(move):
        ...
        case 'うたかたのアリア':
            if defender.ailment == Ailment.BRN:
                ...
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, '追加効果 やけど解除'))
                return True
        case 'ぶきみなじゅもん':
            if defender_mgr.expended_moves and (move := defender_mgr.expended_moves[-1]).pp:
                move.add_pp(-3)
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {move} PP-3"))
                return True
        case 'うちおとす' | 'サウザンアロー':
        ...
        case 'きつけ':
            if defender.ailment == Ailment.PAR:
                ...
                self.battle.logger.append(TurnLog(self.battle.turn, dfn, '追加効果 まひ解除'))
                return True
        case 'くらいつく':
        ...
        case 'なげつける':
            match attacker.item.name:
                case 'おうじゃのしるし' | 'するどいキバ':
                    ...
                    if self.battle.turn_mgr._flinch:
                        self.battle.logger.append(TurnLog(self.battle.turn, self.idx, '追加効果 ひるみ'))
                case 'かえんだま':
            ...
            self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"{attacker.item.name_lost}消失"))
            return True
        case 'みずあめボム':
    ...
    match move.name * (not self.battle.turn_mgr._hit_substitute):
        case 'クリアスモッグ':
            if any(defender_mgr.rank):
                ...
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, '追加効果 能力ランクリセット'))
                return True
        case 'ついばむ' | 'むしくい':
            if defender.item.name[-2:] == 'のみ':
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, '追加効果'))
        ...
        case 'どろぼう' | 'ほしがる':
            if not attacker.item.name and defender.item.name and defender_mgr.is_item_removable():
                ...
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {attacker.item}奪取"))
                return True
        case 'はたきおとす':
            if defender.item.name and defender_mgr.is_item_removable():
                defender.item.active = False
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {defender.item.name_lost}消失"))
                return True
        case 'めざましビンタ':
            if defender.ailment == Ailment.SLP:
                defender_mgr.set_ailment(Ailment.NONE)
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, '追加効果 ねむり解除'))
                return True
        case 'やきつくす':
    ...
    match move.name:
        case 'アイアンローラー' | 'アイススピナー':
            if not self.battle.field_mgr.terrain().is_none():
                ...
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, "追加効果 フィールド消失"))
                return True
        case 'おんねん':
            # TODO おんねん実装
            pass
        ...
        case 'キラースピン' | 'こうそくスピン':
            ...
            for field in [SideField.MAKIBISHI, SideField.DOKUBISHI, SideField.STEALTH_ROCK, SideField.NEBA_NET]:
                if self.battle.field_mgr.set_field(field, self.idx, 0):
                    removed.append(field)
            if removed:
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {[str(field) for field in removed]}解除"))
                return True
        case 'くちばしキャノン':
            pass  # TODO くちばしキャノン実装
        case 'コアパニッシャー':
            if not self.is_first_act() and not defender_mgr.is_ability_protected():
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {defender.ability}消失"))
        ...
        case 'でんこうそうげき' | 'もえつきる':
            ...
            self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"追加効果 {move.type}タイプ消失"))
            return True
        case 'とどめばり':
