...
def _winner(self: Battle,
            TOD: bool = False) -> PlayerIndex | int | None:
    """
    勝敗を判定する
    ...
    TOD_scores = [TOD_score(self, i) for i in range(2)]

    if 0 in TOD_scores or TOD:
        self._winner = TOD_scores.index(max(TOD_scores))
        self.logger.add(TurnLog(self.turn, self._winner, "勝ち"))
        self.logger.add(TurnLog(self.turn, not self._winner, "負け"))
        self.logger.add(CommandLog(self.turn, self.turn_mgr.command, self.turn_mgr.switch_history))
