...
from pokebot.core.logger import TurnLog
        ...
        case 'クォークチャージ' | 'こだいかっせい':
            ...
            if self.boost_source == BoostSource.NONE:
                ...
                if not can_boost:
                    ...
                    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, "ブースト解除"))
                return False

            activated = True
            self.battle.logger.add(TurnLog(self.battle.turn, self.idx,
        ...
        case 'へんげんじざい' | 'リベロ' | 'へんしょく':
            if move:
                ...
                if activated:
                    ...
                    user.ability.count = 1
                    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"-> {move.type}タイプ"))
        case 'ポイズンヒール':
