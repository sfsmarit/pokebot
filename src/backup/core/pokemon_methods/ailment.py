...
def _set_ailment(self: ActivePokemonManager,
    ...
    if not ailment.value:
        ...
        battle.logger.add(TurnLog(battle.turn, self.idx, "状態異常解除"))
        return True
    ...
    if dfn_ability.name in ['きよめのしお', 'ぜったいねむり'] or \
        ...
        battle.logger.add(TurnLog(battle.turn, self.idx, dfn_ability.name))
        return False

    # ミストフィールドによる無効
    if battle.field_mgr.terrain(self.idx) == Terrain.MIST:
        battle.logger.add(TurnLog(battle.turn, self.idx, str(Terrain.MIST)))
        return False
    ...
    battle.logger.add(TurnLog(battle.turn, self.idx, str(self.pokemon.ailment)))
