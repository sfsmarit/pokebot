...
def _add_rank(self: ActivePokemonManager,
    ...
    for i, v in enumerate(values):
        ...
        if self.pokemon.item.name == 'クリアチャーム' and by_opponent and v < 0:
            self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{self.pokemon.item}により無効"))
            continue
        ...
        if v < 0 and by_opponent:
            if self.battle.field_mgr.count[SideField.WHITE_MIST][self.idx]:
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{SideField.WHITE_MIST}により無効"))
                continue

            match self.pokemon.ability.name:
                case 'クリアボディ' | 'しろいけむり' | 'メタルプロテクト':
                    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{self.pokemon.ability}により無効"))
                    continue
                case 'フラワーベール':
                    if "くさ" in self.types:
                        self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{self.pokemon.ability}により無効"))
    ...
    if any(reflection) and not chain and opponent_mgr.add_rank(values=reflection, chain=True):
        self.battle.logger.add(TurnLog(self.battle.turn, self.idx, self.pokemon.ability.name))
    ...
    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, Pokemon.rank2str(delta)))
