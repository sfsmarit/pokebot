...
def set_count(self: ActivePokemonManager, condition: Condition, count: int):
    ...
    if condition in [Condition.CONFUSION]:
        count = ""
    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{condition} {count}"))
    ...
    match condition:
        case Condition.ENCORE:
            if ability.name == 'アロマベール':
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{ability}により無効"))
                return False
        ...
        case Condition.NEMUKE:
            ...
            if self.battle.field_mgr.count[SideField.SHINPI][self.idx]:
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{SideField.SHINPI}により無効"))
            ...
            if self.battle.field_mgr.terrain(self.idx) == Terrain.ELEC:
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{Terrain.ELEC}により無効"))
                return False
        ...
        case Condition.YADORIGI:
            if "くさ" in self.types:
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, "タイプ無効"))
                return False
