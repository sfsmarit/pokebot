    ...
    match tag:
        ...
        case 'rage':
            if self.forced_turn == 0:
                ...
                self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{move}状態"))
            else:
                # ターン経過
                self.forced_turn -= 1
                if self.forced_turn:
                    ...
                    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, f"{move}状態解除"))
        ...
        case 'wall_break':
            ...
            if self.battle.turn_mgr.damage_dealt[self.idx]:
                ...
                if broken:
                    self.battle.logger.add(TurnLog(self.battle.turn, self.idx, '壁破壊'))
                    return True
