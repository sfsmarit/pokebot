...
def _charge_move(self: TurnManager,
    ...
    battle = self.battle
    attacker = battle.pokemons[idx]
    attacker_mgr = battle.poke_mgrs[idx]
    ...
    if ("solar" in move.tags and battle.field_mgr.weather(idx) == Weather.SUNNY) or \
        ...
        battle.logger.add(TurnLog(battle.turn, idx, '溜め省略'))
    elif attacker.item.name == 'パワフルハーブ' and attacker_mgr.activate_item():
        pass
    else:
        battle.logger.add(TurnLog(battle.turn, idx, '行動不能 溜め'))
        return False
