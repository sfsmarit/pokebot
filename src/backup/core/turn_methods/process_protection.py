from __future__ import annotations
from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from ..turn_manager import TurnManager

from pokebot.common.types import PlayerIndex
from pokebot.common.enums import MoveCategory
from pokebot.model import Move
from pokebot.logger import TurnLog
from pokebot.core.move_utils import effective_move_category


def _process_protection(self: TurnManager,
                        atk: PlayerIndex | int,
                        move: Move) -> bool:
    """
    まもる系の技の処理を行い、まもるが成功したらTrueを返す

    Parameters
    ----------
    atk_idx : int
        攻撃側のプレイヤーインデックス
    move : Move
        攻撃技

    Returns
    ----------
    bool
        まもるが成功したらTrue
    """
    dfn = int(not atk)
    attacker = self.battle.pokemons[atk]
    attacker_mgr = self.battle.poke_mgrs[atk]
    defender_mgr = self.battle.poke_mgrs[dfn]

    # まもる貫通
    if 'anti_protect' in move.tags or \
            (attacker.ability.name == 'ふかしのこぶし' and attacker_mgr.contacts(move)):
        self.move_succeeded[dfn] = False
        return False

    move_category = effective_move_category(self.battle, atk, move)

    # 攻撃技かどうか
    self.move_succeeded[dfn] = move_category != MoveCategory.STA

    # 相手に影響を与える変化技かどうか
    if self._protecting_move.name != 'かえんのまもり':
        self.move_succeeded[dfn] |= "protect" in move.tags

    # まもる失敗
    if not self.move_succeeded[dfn]:
        return False

    # 接触時の追加効果
    if attacker_mgr.contacts(move):
        defender_mgr.activate_move_effect(self._protecting_move)

    # 攻撃失敗による反動
    attacker_mgr.apply_move_recoil(move, 'mis_recoil')

    attacker_mgr.forced_turn = 0
    self.battle.logger.add(TurnLog(self.battle.turn, atk, f"{self._protecting_move}で防がれた"))
    self.battle.logger.add(TurnLog(self.battle.turn, dfn, f"{self._protecting_move}成功"))

    return True
