...
def _advance_turn(self: TurnManager,
    ...
    if self.battle.turn == 0:
        ...
        # コマンドを記録
        self.battle.logger.add(CommandLog(self.battle.turn, self.command, self.switch_history))
    ...
    if not any(self.breakpoint):
        ...
        for idx in range(2):
            ...
            self.battle.logger.add(TurnLog(self.battle.turn, idx, text))
            self.battle.logger.add(TurnLog(self.battle.turn, idx,
                                           f"HP {self.battle.pokemons[idx].hp}/{self.battle.pokemons[idx].stats[0]}"))
            self.battle.logger.add(TurnLog(self.battle.turn, idx, f"コマンド {self.command[idx].name}"))
    ...
    if not any(self.breakpoint):
        ...
        for idx in range(2):
            if self.command[idx].is_terastal:
                ...
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"テラスタル {self.battle.pokemons[idx].terastal}"))
        ...
        if not any(self.breakpoint):
            ...
            if "U_turn" in move.tags:
                if move.name in ['クイックターン', 'とんぼがえり', 'ボルトチェンジ'] and ejectbutton_triggered:
                    self.battle.logger.add(TurnLog(self.battle.turn, idx, f"交代失敗"))
