...
def _process_turn_action(self: TurnManager, idx: PlayerIndex | int):
    ...
    if self.command[idx] == Command.SKIP:
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動スキップ'))
    ...
    # 反動で動けない
    if not move.name:
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 反動'))
        self.battle.poke_mgrs[idx].no_act()
        return
    ...
    # こおり判定
        ...
        if "unfreeze" in move.tags or self.battle.random.random() < 0.2:
            ...
            self.battle.poke_mgrs[idx].no_act()
            self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 こおり'))
            return
    ...
    if attacker.ability.name == 'なまけ':
        ...
        if attacker.ability.count % 2 == 0:
            self.battle.poke_mgrs[idx].no_act()
            self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 なまけ'))
            return

    # ひるみ判定
    if self._flinch:
        self.battle.poke_mgrs[idx].no_act()
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 ひるみ'))
    ...
    if self.battle.poke_mgrs[idx].forced_turn == 0:
        ...
        if not is_choosable:
            self.battle.poke_mgrs[idx].no_act()
            self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{move} 不発 ({reason})"))
            return
    ...
    if attacker.ailment == Ailment.PAR and self.battle.random.random() < 0.25:
        self.battle.poke_mgrs[idx].no_act()
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 しびれ'))
        return

    # メロメロ判定
    if self.battle.poke_mgrs[idx].count[Condition.MEROMERO] and self.battle.random.random() < 0.5:
        self.battle.poke_mgrs[idx].no_act()
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '行動不能 メロメロ'))
        return
    ...
    if move.name != 'わるあがき':
        ...
        if self.battle.poke_mgrs[idx].forced_turn == 0:
            ...
            self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{move} {move.pp}/{move._org_pp}"))
    ...
    if move.name == 'ねごと':
        if self.can_execute_move(idx, move):
            ...
            move.observed = True  # 観測
            self.battle.logger.add(TurnLog(self.battle.turn, idx, f"ねごと -> {move}"))
        else:
    ...
    if not self.move_succeeded[idx]:
        self.battle.logger.add(TurnLog(self.battle.turn, idx, "技失敗"))
        return
    ...
    if self._n_strikes > 1:
        self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{self._n_strikes}発"))
    ...
    for cnt in range(self._n_strikes):
        ...
        if not is_hit:
            if cnt == 0:
                ...
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{cnt}ヒット"))
            break
    ...
    self.battle.logger.add(TurnLog(self.battle.turn, idx, f"技{'成功' if self.move_succeeded[idx] else '失敗'}"))
