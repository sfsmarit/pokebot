...
def _process_status_move(self: TurnManager,
    ...
    if self._move_was_mirrored:
        ...
        battle.logger.add(TurnLog(battle.turn, dfn, defender.ability.name))
    ...
    match move.name:
        ...
        case 'うらみ':
            ...
            if self.move_succeeded[atk]:
                ...
                battle.logger.add(TurnLog(battle.turn, atk, f"{expended_move} PP {expended_move.pp}"))
        case 'エレキフィールド':
        ...
        case 'スキルスワップ':
            ...
            if self.move_succeeded[atk]:
                ...
                for i in range(2):
                    battle.logger.add(TurnLog(battle.turn, i, f"-> {battle.pokemons[i].ability}"))
                # 特性の再発動
        ...
        case 'すりかえ' | 'トリック':
            ...
            if self.move_succeeded[atk]:
                ...
                for i, poke in enumerate(battle.pokemons):
                    battle.logger.add(TurnLog(battle.turn, i, f"-> {poke.item}"))
                    # 即時発動アイテムの判定
        ...
        case 'テクスチャー':
            ...
            if self.move_succeeded[atk]:
                ...
                battle.logger.add(TurnLog(battle.turn, atk, f"-> {attacker_mgr.types[0]}タイプ"))
        case 'でんじふゆう':
        ...
        case 'ひっくりかえす':
            ...
            if self.move_succeeded[atk]:
                ...
                self.move_succeeded[atk] = dfn == org_dfn
                battle.logger.add(TurnLog(battle.turn, atk, f"-> {Pokemon.rank2str(defender_mgr.rank)}"))
        case 'ビルドアップ':
        ...
        case 'ミラータイプ':
            ...
            if self.move_succeeded[atk]:
                ...
                battle.logger.add(TurnLog(battle.turn, atk, f"-> {attacker_mgr.types}タイプ"))
        case 'みがわり':
        ...
        case 'リサイクル':
            ...
            if self.move_succeeded[atk]:
                ...
                battle.logger.add(TurnLog(battle.turn, atk, f"{attacker.item}回収"))
        case 'リフレッシュ':
            self.move_succeeded[atk] = attacker_mgr.set_ailment(Ailment.NONE)
        case 'ロックオン':
