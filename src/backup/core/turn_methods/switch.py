...
def _switch_pokemon(self: TurnManager,
    ...
    if poke and poke.name == 'イルカマン(ナイーブ)':
        ...
        self.battle.logger.add(TurnLog(self.battle.turn, idx, '-> マイティフォルム'))
    ...
    self.battle.logger.add(TurnLog(self.battle.turn, idx, self.battle.command_to_str(idx, command)))
    ...
    if opponent:
        ...
        if active_ability.name == 'かがくへんかガス':
            ...
            self.battle.logger.add(TurnLog(self.battle.turn, opp, 'かがくへんかガス解除'))
    ...
    if baton:
        ...
        for key in baton:
            if isinstance(key, Condition):
                ...
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"継承 {key} {baton[key]}"))
            elif key == 'sub_hp':
                poke_mgr.sub_hp = baton[key]
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"継承 みがわり HP{baton[key]}"))
            elif key == 'rank':
                poke_mgr.rank = baton[key]
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"継承 ランク {baton[key][1:]}"))
            ...
            if not poke_mgr.is_floating():
                ...
                if self.battle.field_mgr.count[SideField.DOKUBISHI][idx]:
                    if 'どく' in poke_mgr.types:
                        ...
                        self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{SideField.DOKUBISHI}解除"))
                    elif poke_mgr.set_ailment(Ailment.PSN, bad_poison=(self.battle.field_mgr.count[SideField.DOKUBISHI][idx] == 2)):
                        self.battle.logger.add(TurnLog(self.battle.turn, idx, f"{SideField.DOKUBISHI}接触"))
    ...
    for idx in idxes:
        ...
        if not poke_mgr.is_ability_protected():
            ...
            if opponent.ability.name == 'かがくへんかガス' and poke.item.name != 'とくせいガード':
                ...
                self.battle.logger.add(TurnLog(self.battle.turn, idx, 'かがくへんかガス 特性無効'))
                break

            # トレース
            if poke.ability.name == 'トレース' and "unreproducible" not in opponent.ability.flags:
                poke.ability.name = opponent.ability.name
                self.battle.logger.add(TurnLog(self.battle.turn, idx, f"トレース -> {opponent.ability}"))
