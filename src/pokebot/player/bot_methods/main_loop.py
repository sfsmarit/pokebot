    ...
    while True:
        if self._read_phase().value:
            ...
            if not self.online:
                ...
                elif time.time() - self.nonephase_start_time > Time.OUT.value:
        ...
        match self.phase:
            ...
            case Phase.SELECTION:
                ...
                # コマンド入力
                t0 = time.time()
                self._input_selection_command(commands)
                dt += time.time() - t0
            ...
            case Phase.ACTION:
                ...
                # 盤面の状態を取得
                if not self._read_banmen():
                    warnings.warn('Failed to read Banmen')
                    type(self).press_button('B', n=5)
                else:
                    self._process_text_buffer()  # バッファを処理
                    ...
                    # コマンド入力
                    if self._input_action_command(command):
                        dt += time.time() - t0
                        print(f"操作時間 {dt:.1f}s")
            ...
            case Phase.SWITCH:
                ...
                self._process_text_buffer()  # テキストバッファ処理
                ...
                # コマンド入力
                self._input_switch_command(command)
            ...
            case _:
                ...
                # 画面のテキストを取得
                if self._read_screen_text(capture=False):
                    # 特性テキストも取得
                    for pidx in range(2):
                        self._read_ability_text(pidx, capture=False)
