...
def _read_pokemon_from_box(self: Bot):
    ...
    img = iut.BGR2BIN(self.img[580:620, 1455:1785], threshold=180, bitwise_not=True)
    ability_name = iut.OCR(img, candidates=PokeDB.abilities, log_dir=type(self).ocr_log_dir / "box_ability")

    # 名前
    img = iut.BGR2BIN(self.img[90:130, 1420:1620], threshold=180, bitwise_not=True)
    label = iut.OCR(img, candidates=list(PokeDB.label_to_names.keys()), log_dir=type(self).ocr_log_dir / "box_name")
        ...
        for s in PokeDB.label_to_names[label]:
            ...
            if PokeDB.form_diff[label] == 'type':
                ...
                for t in range(2):
                    ...
                    t = iut.OCR(img, candidates=TYPES, log_dir=type(self).ocr_log_dir / "box_type")
                    types.append(t)
    ...
    # もちもの
    img = iut.BGR2BIN(self.img[635:685, 1455:1785], threshold=180, bitwise_not=True)
    poke.item = Item(iut.OCR(img, candidates=PokeDB.items() + [''], log_dir=type(self).ocr_log_dir / "box_item"))

    # テラスタイプ
    x0 = 1535+200*(len(poke._types)-1)
    img = iut.BGR2BIN(self.img[154:186, x0:x0+145], threshold=240, bitwise_not=True)
    poke.terastal = iut.OCR(img, candidates=TYPES, log_dir=type(self).ocr_log_dir / "box_terastal")
    ...
    # レベル
    img = iut.BGR2BIN(self.img[25:55, 1775:1830], threshold=180, bitwise_not=True)
    poke.level = int(iut.OCR(img, log_dir=type(self).ocr_log_dir / "box_level/", lang='num').replace('.', ''))
    ...
    for j in range(6):
        img = iut.BGR2BIN(self.img[y[j]:y[j]+45, x[j]:x[j]+155], threshold=180, bitwise_not=True)
        s = iut.OCR(img, lang=('eng' if j == 0 else 'num'))
        if j == 0:
            s = s[s.find('/')+1:]
        stats[j] = int(s)
