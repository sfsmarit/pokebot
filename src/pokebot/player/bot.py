...
from .bot_methods.real_game import _real_game
...
class Bot(Player):
    ...
    @classmethod
    def init_bot(cls, video_id: int):
        """VideoCapture, nxbtを初期化する"""
        TemplateImage.init()
        ...
        print("\t(Video ID を確認するコマンド: v4l2-ctl --list-devices)\n")
    ...
    def game(self,
        ...
        online : bool, optional
            Trueならオンライン対戦、Falseなら学校最強大会, by default True
        ...
        self.online = online
        n_selection = 3 if online else len(self.team)
        if seed is None:
            seed = int(time.time())

        self.battle = Battle(self, opponent, n_selection=n_selection, seed=seed)

        # オフライン対戦なら全員選出
        if not self.online and self.battle:
            self.battle.selection_indexes[0] = list(range(len(self.team)))
    ...
    def input_selection_command(self, commands: list[Command]) -> list[int]:
        return _input_selection_command(self, commands)

    def input_action_command(self, command: Command) -> bool:
        if command == Command.STRUGGLE:
            ...
            # 交代画面に移動
            while True:
                if (pos := self.action_cursor_position()) == 1:
                    break
                delta = 1 - pos
            ...
            if self.is_switch_window():
                return self.input_switch_command(command)
        else:
            return _input_move_command(self, command)

        return False

    def input_switch_command(self, command: Command) -> bool:
        return _input_switch_command(self, command)

    def read_phase(self, capture: bool = True) -> Phase:
        """対戦のフェーズを読み取る"""
        if self.is_action_window(capture=capture):
            ...
            self.battle.phase = Phase.SELECTION
        elif self.is_standby_window(capture=False):
            self.battle.phase = Phase.STAND_BY
        else:
            self.battle.phase = Phase.NONE
        return self.battle.phase

    def is_standby_window(self, capture: bool = True) -> bool:
        """オンライン対戦の待機画面ならTrue"""
        if capture:
            type(self).capture()
    ...
    def is_selection_window(self, capture: bool = True) -> bool:
        """選出画面ならTrue"""
        if capture:
            type(self).capture()
    ...
    def is_action_window(self, capture: bool = True) -> bool:
        """ターン開始時の画面ならTrue"""
        if capture:
            type(self).capture()
    ...
    def is_switch_window(self, capture: bool = True) -> bool:
        """交代画面ならTrue"""
        if capture:
            type(self).capture()
    ...
    def is_condition_window(self, capture: bool = True) -> bool:
        """場の状態の確認画面ならTrue"""
        if capture:
            type(self).capture()
    ...
    def selection_cursor_position(self, capture: bool = True) -> int:
        """選出画面のカーソル位置"""
        if capture:
            type(self).capture()
        for i in range(6):
    ...
    def action_cursor_position(self, capture: bool = True) -> int:
        """
        行動選択画面のカーソル位置
    ...
    def move_cursor_position(self, capture: bool = True) -> int:
        """技選択画面のカーソル位置"""
        if capture:
            type(self).capture()
        for i in range(4):
    ...
    def read_pp(self, move_idx, capture: bool = True) -> int:
        """技選択画面からPPを読み取る"""
        if capture:
            type(self).capture()
    ...
    def _read_switch_state(self, capture: bool = True) -> str:
        ...
        for s, template in TemplateImage.switch_state.items():
    ...
    def read_screen_text(self, capture: bool = True) -> bool:
        """画面テキストを読み取る"""
        return _read_screen_text(self, capture)

    def read_ability_text(self, idx: PlayerIndex | int, capture: bool = True):
        """特性テキストを読み取る"""
    ...
    def read_banmen(self) -> bool:
        """盤面の状態を読み取る"""
        return _read_banmen(self)

    def read_opponent_terastal(self, capture: bool = True) -> str:
        """発動中の相手のテラスタイプを読み取る"""
        return _read_opponent_terastal(self, capture)

    def read_active_label(self, idx: PlayerIndex | int, capture: bool = True):
        """場のポケモンの表示名を読み取る"""
        return _read_active_label(self, idx, capture)

    def read_hp(self, capture: bool = True) -> int:
        """場のポケモンのHPを読み取る"""
        return _read_hp(self, capture)

    def read_hp_ratio(self, capture: bool = True) -> float:
        """場のポケモンのHP割合を読み取る"""
        return _read_hp_ratio(self, capture)

    def read_ailment(self, capture: bool = True) -> Ailment:
        """場のポケモンの状態異常を読み取る"""
    ...
    def read_form(self, label: str, capture: bool = True) -> str | None:
        """場のポケモンのフォルムを読み取る"""
        return _read_form(self, label, capture)

    def read_fainting_opponent(self, capture: bool = True):
        """相手の瀕死ポケモンを読み取る"""
        return _read_fainting_opponent(self, capture)

    def overwrite_condition(self, idx: PlayerIndex | int, count: dict):
        """場の状態を上書きする"""
        # オーロラベールを両壁に書き換える
        if SideField.AURORA_VEIL.name in count:
            count[SideField.REFLECTOR] = count[SideField.AURORA_VEIL]
            count[SideField.LIGHT_WALL] = count[SideField.AURORA_VEIL]
