...
def _process_tagged_move(self: ActivePokemonManager,
    ...
    match tag:
        ...
        case 'immovable':
            # 反動で行動できない技
            self.unresponsive_turn = 1
            return True

        case 'rage':
            if self.unresponsive_turn == 0:
                # あばれる状態の付与
                self.unresponsive_turn = self.battle.random.randint(1, 2)
                self.battle.logger.append(TurnLog(self.battle.turn, self.idx, f"{move} 残り{self.unresponsive_turn}ターン"))
            else:
                # ターン経過
                self.unresponsive_turn -= 1
                # こんらん付与
