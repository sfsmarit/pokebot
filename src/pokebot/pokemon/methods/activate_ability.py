    ...
    match user.ability.name:
        ...
        case 'クォークチャージ' | 'こだいかっせい':
            ...
            self.battle.logger.append(TurnLog(self.battle.turn, self.idx,
                                              f"{STAT_CODES[user.boosted_idx]}上昇"))  # type: ignore
        case 'くだけるよろい':
        ...
        case 'バリアフリー':
            for i in range(2):
                activated |= self.battle.field_mgr.set_field(SideField.REFLECTOR, PlayerIndex(i), 0)
                activated |= self.battle.field_mgr.set_field(SideField.LIGHTWALL, PlayerIndex(i), 0)
        case 'ばんけん':
            activated = opponent_mgr.add_rank(1, +1, by_opponent=True)
        case 'はんすう':
            activated = True
