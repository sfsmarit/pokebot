...
def _end_turn(self: TurnManager):
    ...
    for idx in self.speed_order:
        if battle.field_mgr.terrain(idx) == Terrain.GRASS and \
                ...
                battle.pokemon[idx].hp and \
                battle.poke_mgr[idx].add_hp(ratio=1/16):
    ...
    for idx in self.speed_order:
        ...
        if poke.hp and poke.item.name in ['たべのこし', 'くろいヘドロ']:
            battle.poke_mgr[idx].activate_item()
            if battle.winner() is not None:
                return
    ...
    for idx in self.speed_order:
        ...
        poke_mgr = battle.poke_mgr[idx]
        opponent_mgr = battle.poke_mgr[opp]
    ...
    for idx in self.speed_order:
        ...
        if poke.hp and poke_mgr.count[Condition.BIND]:
            ...
            battle.logger.append(TurnLog(battle.turn, idx, f"バインド 残り{int(poke_mgr.count[Condition.BIND])}ターン"))
    ...
    for idx in self.speed_order:
        ...
        if poke.hp and poke_mgr.add_condition_count(Condition.HOROBI, -1):
            if poke_mgr.count[Condition.HOROBI] == 0:
                poke.hp = 0
                battle.logger.append(TurnLog(battle.turn, idx, 'ほろびのうた 瀕死'))
                if battle.winner() is not None:
                    return
    ...
    for idx in self.speed_order:
        ...
        if poke_mgr.executed_move.name == 'はねやすめ' and \
            ...
            battle.logger.append(TurnLog(battle.turn, idx, 'はねやすめ解除'))
