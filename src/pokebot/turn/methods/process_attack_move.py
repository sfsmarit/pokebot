...
from pokebot.move import critical_probability
...
def _process_attack_move(self: TurnManager,
    ...
    # 急所判定
    critical_prob = critical_probability(self.battle, atk, move)
    critical = self.battle.random.random() < critical_prob
    if critical:
        self.battle.logger.append(TurnLog(self.battle.turn, atk, '急所'))
    ...
    if self._hit_substitute:
        ...
        if defender_mgr.sub_hp:
            self.battle.logger.append(TurnLog(self.battle.turn, dfn, f"みがわりHP {defender_mgr.sub_hp}"))
        else:
        ...
        self.battle.logger.append(TurnLog(self.battle.turn, atk, f"ダメージ {self.damage_dealt[atk]}"))
    ...
    if move.name in PokeDB.move_effect:
        ...
        tgt = PlayerIndex((atk + effect["target"]) % 2)
        target_mgr = self.battle.poke_mgr[tgt]
        ...
        if (tgt == atk or defender_mgr.can_receive_move_effect(move)) and \
            ...
            if effect['confusion']:
                if target_mgr.set_condition(Condition.CONFUSION, self.battle.random.randint(2, 5)):
                    self.battle.logger.append(TurnLog(self.battle.turn, atk, '追加効果 こんらん'))
    ...
    if defender_mgr.count[Condition.MICHIZURE] and defender.hp == 0:
        attacker.hp = 0
        self.battle.logger.append(TurnLog(self.battle.turn, atk, '瀕死 みちづれ'))
