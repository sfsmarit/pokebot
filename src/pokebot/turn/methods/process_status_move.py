...
def _process_status_move(self: TurnManager,
    ...
    org_dfn = dfn = PlayerIndex(not atk)
    ...
    match move.name:
        ...
        case 'いたみわけ':
            ...
            for i in range(2):
                battle.poke_mgr[i].add_hp(h - battle.pokemon[i].hp, move=move)
        ...
        case 'おかたづけ' | 'りゅうのまい':
            ...
            if move.name == 'おかたづけ':
                for field in [SideField.MAKIBISHI, SideField.DOKUBISHI, SideField.STEALTH_ROCK, SideField.NEBA_NET]:
                    for i in range(2):
                        self.move_succeeded[atk] |= battle.field_mgr.set_field(field, PlayerIndex(i), 0)
        case 'おきみやげ':
        ...
        case 'くろいきり':
            self.move_succeeded[atk] = False
            for i in range(2):
                self.move_succeeded[atk] |= any(battle.poke_mgr[i].rank)
                battle.poke_mgr[i].reset_rank()
        case 'くろいまなざし' | 'とおせんぼう':
        ...
        case 'スキルスワップ':
            ...
            if self.move_succeeded[atk]:
                ...
                for i in range(2):
                    battle.logger.append(TurnLog(battle.turn, i, f"-> {battle.pokemon[i].ability}"))
                # 特性の再発動
                for i in battle.turn_mgr.speed_order:
                    if "immediate" in battle.pokemon[i].ability.tags:
        ...
        case 'すりかえ' | 'トリック':
            ...
            if self.move_succeeded[atk]:
                ...
                for i, poke in enumerate(battle.pokemon):
                    poke.item.observed = True
