...
def setup_ability(
    ...
    name_lists = [ally, foe]
    ability_lists = [[ability], [opp_ability]]
...
def test():
    battle = setup_ability("かちき")
    battle.modify_stat(battle.actives[0], Stat.A, -1, by_foe=True)
    assert battle.actives[0].ability.observed
    assert battle.actives[0].field_status.rank[Stat.C.idx] == 2
    battle = setup_ability("かちき")
    battle.modify_stat(battle.actives[0], Stat.A, -1, by_foe=False)
    assert not battle.actives[0].ability.observed
    assert battle.actives[0].field_status.rank[Stat.C.idx] == 0
    battle = setup_ability("かちき", opp_ability="いかく")
    assert battle.actives[0].field_status.rank[Stat.C.idx] == 2
