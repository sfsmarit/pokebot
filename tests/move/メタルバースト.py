...
def メタルバースト(display_log: bool = False) -> bool:
    moves = ["メタルバースト", "ほうふく"]
    opponents = ["ミュウツー", "カメックス"]
    results = ["技成功", "技失敗"]

    result = True
    for move, opp, res in zip(moves, opponents, results):
        single_test_result = test(move, opp, res, display_log)
        if display_log:
...
def test(move: str, opp: str, res: str, display_log: bool = False) -> bool:
    max_turn = 1
    ...
    return res in "".join(battle.logger.get_turn_log(turn=battle.turn, idx=0))
